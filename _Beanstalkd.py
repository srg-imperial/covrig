...
class Beanstalkd(Container):
    ...
    def __init__(self, _image, _user, _pwd):
        ...
        self.tsuite_path = (
            '/home/beanstalkd/testheap.c', '/home/beanstalkd/testjobs.c',
    ...
    def compile(self):
        ...
        with cd(self.path):
            with settings(warn_only=True):
                ...
                result = run(
                    "make CFLAGS='-O0 --coverage -Wl,--no-as-needed -levent -lrt' LDFLAGS='--coverage -Wl,--no-as-needed -levent -lrt'")
                if result.failed:
    ...
    def make_test(self):
        ...
        if not self.compileError:
            with cd(self.path):
                with settings(warn_only=True):
                    result = run(
                        "make check CFLAGS='-O0 --coverage -Wl,--no-as-needed -levent -lrt' LDFLAGS='--coverage -Wl,--no-as-needed -levent -lrt'")
                    if result.failed:

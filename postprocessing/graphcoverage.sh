...
egrep -v "$IGNOREREVS" $INPUT |awk 'BEGIN { FS="," } ; { if ($2 > 0) print NR,$3/$2; }' >$DATAFILE
...
set title \"${OUTPUT:11}\"" $GRAPHTYPE
  ...
  "$SCRIPT_DIR/internal/lineplot.sh" $DATAFILE "$OUTPUT" "Revision" "Coverage" "set title \"${OUTPUT:11}\"" $GRAPHTYPE
fi
#rm -f $DATAFILE

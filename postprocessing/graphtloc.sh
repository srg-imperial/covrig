...
if [[ $GRAPHTYPE == "standalone" ]]; then
  "$SCRIPT_DIR/internal/lineplot.sh" $DATAFILE "$OUTPUT" "Revision" "TLOC" "set size 0.8,0.8
set title \"${OUTPUT:11}\"" $GRAPHTYPE
else
  "$SCRIPT_DIR/internal/lineplot.sh" $DATAFILE "$OUTPUT" "Revision" "TLOC" "set title \"${OUTPUT:11}\"" $GRAPHTYPE
fi

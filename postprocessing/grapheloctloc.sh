...
if [[ $GRAPHTYPE == "standalone" ]]; then
  "$SCRIPT_DIR/internal/lineplot2yaxes.sh" $DATAFILE "$OUTPUT" "Revision" "ELOC" "set size 0.8,0.8
set title \"${OUTPUT:11}\"
set yrange [ 0 : ]
set y2label \"TLOC\"" $GRAPHTYPE
else
  "$SCRIPT_DIR/internal/lineplot2yaxes.sh" $DATAFILE "$OUTPUT" "Revision" "ELOC" "set yrange [ 0 : ]
set title \"${OUTPUT:11}\"
set y2label \"TLOC\"" $GRAPHTYPE
fi
#rm -f $DATAFILE

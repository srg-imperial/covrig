...
# Collector() is passed to DataHandler(), which dumps data to a CSV output
# file
...
class Analytics(object):
    ...
    @classmethod
    def run_custom(cls, _pclass, _image, _commit, _count,
                   _startaftercommit=None, _maxcommits=0):
    ...
    def go(self, outputfolder, outputfile):
        ...
        # check oldest commit first. this makes it easier to check patch
        # coverage in subsequent versions
        ...
        for i in self.commits:
            ...
            try:
                ...
                for i, (files, lines) in enumerate(prev_uncovered_list):
                    prev_uncovered_list[
                        i] = c.prev_patch_coverage(
                        i,
                        files,
                        lines)

                print (c.changed_files, c.uncovered_lines_list)
                prev_uncovered_list.insert(
                    0, (c.changed_files, c.uncovered_lines_list))
...
def main():
    ...
    parser.add_argument(
        '--limit',
        type=int,
        help="limit to n number of revisions")
    ...
    try:
        ...
        container = Analytics.run_custom(
            b["class"],
            image,
            b["revision"],
            args.revisions if args.revisions else b["n"],
            lastrev,
            args.limit)

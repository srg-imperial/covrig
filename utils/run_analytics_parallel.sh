...
analytics(){
  ...
  echo "============================"
  # If the repeats is not specified, don't provide --repeats
  if [ -z "$REPEATS" ]; then
...
parallel --results "data/""$REPO""_logs" --link -j "$NUM_PROCESSES" -k analytics {1} {2} "$REPO" {3} "$IMAGE" "$FINAL_COMMIT" "$REPEATS" ::: "${OUTPUT_FILES[@]}" ::: "${NCPP_ARRAY[@]}" ::: "${COMMIT_RANGES[@]}" &
pid=$!

...
class Container(object):
    ...
    def count_sloc(self, path):
        ...
        for p in path:
            try:
                lines += int(self.omnirun("cloc " + p +
                             " | tail -2 | awk '{print $5}'"))
            except ValueError:
                lines += int(self.omnirun("wc -l " + p +
                             "/*|tail -1|awk '{ print $1 }'"))
        return str(lines)
    ...
    def checkout(self, prev_revision, revision):
        ...
        with self.omnicd(self.path):
            ...
            diffcmd = "git diff -b --pretty='format:' --name-only " + \
                prev_revision + " " + self.revision + " -- "
    ...
    def overall_coverage(self):
        ...
        if self.offline:
            ...
            with self.omnicd(covdatadir), settings(warn_only=True):
                res = local(
                    'rm -rf tmp && mkdir tmp && tar xjf coverage-%s.tar.bz2 -C tmp' %
                    self.revision)
                if res.failed:
                    return
                covdatadir += "/tmp"
        ...
        with self.omnicd(covdatadir), settings(warn_only=True):
            ...
            if hasattr(self, 'ignore_coverage_from'):
                ignore = ' '.join(
                    ["'%s'" %
                     p for p in self.ignore_coverage_from])
                self.omnirun('lcov -r total.info %s -o total.info' % ignore)
            lines = self.omnirun(
                "lcov --summary total.info 2>&1|tail -3|head -1|sed 's/.*(//' |egrep -o '[0-9]+'")
    ...
    def has_coverage_information(self, filepath):
        if self.offline:
            covdatadir = '%s/data/%s/tmp' % (self.initialpath,
                                             self.outputfolder)
        else:
            covdatadir = self.source_path
        ...
        with self.omnicd(covdatadir):
            result = self.omnirun(
                "sed -n '\|SF:.*/%s|,/end_of_record/p' total.info" %
                filepath)
        return bool(result)
    ...
    def is_covered(self, filepath, line):
        ...
        with self.omnicd(covdatadir), settings(warn_only=True):
            result = self.omnirun(
                "sed -n '\|SF:.*/%s|,/end_of_record/p' total.info |grep '^DA:%d,'" %
                (filepath, line))
            if not result:
    ...
    def patch_coverage(self, prev_revision):
        ...
        with self.omnicd(self.path):
            ...
            if changed_files:
                self.changed_files = [
                    i for i in changed_files.splitlines() if i]
                            ...
                            if fileExists == 'y':
                                ...
                                for td in self.tsuite_dir:
                                    if realp.startswith(td + "/"):
                                        self.changed_test_files.append(f)
                        self.changed_test_files = list(
                            set(self.changed_test_files))
                        if self.has_coverage_information(f):
                            ...
                            with self.omnicd(self.path):
                                file_diff = self.omnirun("git diff -b -U0 " +
                                                         ...
                                                         " -- " + f +
                                                         " | perl -pe 's/\e\[?.*?[\@-~]//g'")
                                self.ehunkheads += [i for i in file_diff.splitlines()
                                                    if i.startswith('@@')]
                                file_diff3 = self.omnirun("git diff -b " +
                                                          prev_revision + " " + self.revision +
                                                          " -- " + f +
                                                          " | perl -pe 's/\e\[?.*?[\@-~]//g'")
                                self.ehunkheads3 += [i for i in file_diff3.splitlines()
                                                     if i.startswith('@@')]
                            line_numbers = self.omnirun(
                                "%s %s %s %s" %
                                (self.difflinessh, prev_revision, self.revision, f))
                            ...
                            # executed by the test suite. Alternatives include:
                            # file was removed, program crashed or has no
                            # permissions
    ...
    def prev_patch_coverage(self, backcnt, prev_files, prev_lines):
        ...
        for i, f in enumerate(prev_files):
            ...
            prev_lines[i][:] = [l for l in prev_lines[i]
                                if self.is_covered(f, l) != self.LineType.Covered]
            covered -= len(prev_lines[i])
    ...
    def collect(self, author_name, timestamp, outputfolder, outputfile):
        ...
        # pass the Collector() obj to the Data Handler to store results in CSV
        # format
        x = DataHandler(c)
        x.extractData()
        x.dumpCSV()
